...
func ProjectStatusColumnsRoutes(controller *api.ProjectStatusColumnsController, authMiddlewares chi.Middlewares, authzMiddleware *auth.AuthorizationMiddleware) chi.Router {
	...
	r.Group(func(r chi.Router) {
		r.Use(authzMiddleware.RequireResourceAccessFromBody(auth.ResourceTypeProject, auth.ExtractProjectIDFromBody))
	...
	r.Group(func(r chi.Router) {
		r.Use(authzMiddleware.RequireResourceAccess(auth.ResourceTypeProjectStatusColumn, "id"))
	...
	r.Group(func(r chi.Router) {
		r.Use(authzMiddleware.RequireResourceAccess(auth.ResourceTypeProject, "project_id"))
